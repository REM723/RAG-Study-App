...
from ..db import Attempt, Question, Test, get_db
from ..schemas import AttemptRequest, TestQuestionOut, TestRequest, TestView

router = APIRouter(prefix="/tests", tags=["tests"])


def _pick(db, qtype, n):
    if n <= 0:
        return []
    rows = db.query(Question).filter_by(type=qtype).all()
    if len(rows) < n:
        raise HTTPException(
...
def _view(db, test):
    ...
    return {"id": test.id, "questions": [TestQuestionOut.model_validate(q) for q in qs]}
...
def create_test(req: TestRequest, db: Session = Depends(get_db)):
    ...
    qs = _pick(db, "mcq", req.mcq_count) + _pick(db, "descriptive", req.descriptive_count)
    test = Test(question_ids=[q.id for q in qs])
    db.add(test)
    db.commit()
    db.refresh(test)
...
def submit(test_id: int, req: AttemptRequest, db: Session = Depends(get_db)):
    ...
    a.answers, a.submitted, a.result = answers, True, result
    db.commit()
    return result
